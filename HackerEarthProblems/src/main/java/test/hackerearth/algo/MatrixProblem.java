...
public class MatrixProblem {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 1; i < allsubMatrixes; i++) {
      ...
      if (column == 0) sum = subMatrixSumFromStartingColumn(inputMatrix, row, column, subMatSize);
      else sum = subMatrixSubFromPreviousSum(inputMatrix, row, column, subMatSize, sum);
      if (isCube(sum)) result++;
    }
  ...
  private static int subMatrixSubFromPreviousSum(
      int[][] m, int row, int column, int k, int previousSum) {

...
public class Trip {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < testcases; i++) {
      ...
      for (int j = 0; j < q; j++) {
        ...
        personsMaps
            .get(moneyPaidByPerson)
            .setPaidMoney(personsMaps.get(moneyPaidByPerson).getPaidMoney() + transactionAmount);
        personsMaps
            .get(moneyPaidByPerson)
            .setShare(
                personsMaps.get(moneyPaidByPerson).getShare()
                    + sharingAmount
                    + (transactionAmount % (numberOfPeople + 1)));
        for (int k = 0; k < numberOfPeople; k++) {
          String otherPerson = br.readLine().trim();
          personsMaps
              .get(otherPerson)
              .setShare(personsMaps.get(otherPerson).getShare() + sharingAmount);
        }
      }
  ...
  private static void showTripPersonDetail(TripPerson person) {
    if (person.getPaidMoney() == person.getShare()) {
      ...
      System.out.println(
          person.getName() + " is owed " + (person.getPaidMoney() - person.getShare()));

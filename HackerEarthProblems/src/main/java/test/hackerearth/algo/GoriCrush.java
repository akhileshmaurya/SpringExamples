    ...
    for (int i = 0; i < q; i++) {
      ...
      if (input[0].trim().equalsIgnoreCase("0")) {
        ...
        for (int j = 0; j < tempList.length; j++) {
          if (a <= Integer.parseInt(tempList[j])
              && c <= Integer.parseInt(tempList[j])
              && b >= Integer.parseInt(tempList[j])
              && d >= Integer.parseInt(tempList[j])) count++;
          else if (b < Integer.parseInt(tempList[j]) || d < Integer.parseInt(tempList[j])) {
            break;
          }
        }
        // System.out.println("count:" + count + " k:" + k);
        if (count >= k) System.out.println("Propose");

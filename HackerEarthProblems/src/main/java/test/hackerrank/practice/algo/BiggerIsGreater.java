...
public class BiggerIsGreater {
		...
		for (int i = 0; i < t; i++) {
			...
			for (int j = input.length() - 1; j > 0; j--) {
				if (isGreater(input.charAt(j), input.charAt(j - 1))) {
					...
					input = new StringBuffer(input).replace(j-1, j, String.valueOf(input.charAt(k))).toString();
	...
	private static int findK(int j, String input) {
		...
		for (int i = j+1; i < input.length(); i++) {
			if((int)input.charAt(i)<(int)input.charAt(k)&& (int)input.charAt(i)>(int)input.charAt(j)){
				k=i;
			}
		}
		return k;

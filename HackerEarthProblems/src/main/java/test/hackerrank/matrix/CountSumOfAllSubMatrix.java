...
public class CountSumOfAllSubMatrix {
	public static void main(String[] args) throws Exception{
		...
		System.out.println("Total Sub Matrix :" +
		calculateAllSubMatrix(mat));
	...
	static long countAllSubMatrixGreterThanSumK(int[][] mat, long sumValue) {
		...
		for (int i = 0; i < noOfRow; i++) {
			for (int j = 0; j < noOfColumn; j++) {
				...
				if (i == 0)
					columnSum[i][j] = mat[i][j];
				else {
					columnSum[i][j] = mat[i][j] + columnSum[i - 1][j];
					int temp = 0;
					for (int index1 = 0; index1 < i; index1++) {
						if (columnSum[i][j] - temp > sumValue)
							totalMatrix++; // matrix 2x1,3x1,...mx1
				...
				if (j == 0)
					...
					for (int index1 = 0; index1 < j; index1++) {
						if (rowSum[i][j] - temp > sumValue)
							totalMatrix++; // matrix 1x2,1x3,...1xn
						temp = rowSum[i][index1];
					}
				}

				if (i == 0 && j == 0)
					rowColumnSum[i][j] = mat[i][j];
				else if (i == 0) {
					...
					rowColumnSum[i][j] = rowColumnSum[i - 1][j] + rowColumnSum[i][j - 1] - rowColumnSum[i - 1][j - 1] + mat[i][j];
					if (rowColumnSum[i][j] > sumValue)
						totalMatrix++;
					for (int index1 = 0; index1 < i; index1++) {
						for (int index2 = 0; index2 < j; index2++) {
							...
							if (index2 > 0)
								val = val - rowColumnSum[i][index2 - 1];
							if (index1 > 0)
								val = val - rowColumnSum[index1 - 1][j];
							if (index1 > 0 && index2 > 0)
								val = val + rowColumnSum[index1 - 1][index2 - 1];
							if (val > sumValue)
								totalMatrix++; // others matrix
						}

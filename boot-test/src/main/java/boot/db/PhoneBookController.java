...
public class PhoneBookController {
	...
	public PhoneBook addPhoneBook(@PathVariable String name,
			@PathVariable String mobile, @PathVariable String type,
			@PathVariable String address) {
		PhoneBook phoneBook = new PhoneBook();
		phoneBook.setName(name);
		phoneBook.setMobile(mobile);
		phoneBook.setTypes(Types.valueOf(type));
		if (!StringUtils.hasText(phoneBook.getTypes().name()))
			phoneBook.setTypes(Types.MOBILE);
